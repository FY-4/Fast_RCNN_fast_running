...
class FRCNN(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        #   设置字体与边框厚度
        #---------------------------------------------------------#
        font_size = np.floor(3e-2 * image.size[1] + 0.5).astype(int)  # 确保结果为整数
        font_path = 'model_data/simhei.ttf'  # 确保这个路径是正确的
        ...
        for i, c in list(enumerate(top_label)):
            ...
            text_bbox = draw.textbbox((0, 0), label, font=font)  # 计算文本边界框
            label_size = (text_bbox[2] - text_bbox[0], text_bbox[3] - text_bbox[1])  # 提取宽高
            label = label.encode('utf-8')
            # print(label, top, left, bottom, right)
            classlist.append(predicted_class)
